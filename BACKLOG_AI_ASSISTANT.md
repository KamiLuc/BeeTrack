...
| AST-17-BE | `BE`  | `[ ]`  | Migration: `assistant_conversations` + `assistant_message_logs` + `assistant_tool_calls` | Widened from an audit-log-only sketch once we decided the app should support "just send the next message" instead of the client resending full history every call (see AST-19-BE) — these tables are now the source of truth for conversation reconstruction, not a side log. `assistant_conversations(id, user_id, created_at)` — one row per conversation. `assistant_message_logs(id, conversation_id, role, content, created_at)` — one row per user turn and per final assistant answer, ordered by `created_at` within a conversation. `assistant_tool_calls(id, message_id, tool_name, input jsonb, result jsonb, is_error, created_at)` — one row per MCP tool call the loop made while producing that assistant row |
