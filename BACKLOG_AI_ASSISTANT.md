...
| AST-02-BE | `BE`  | `[x]`  | Tool: `list_hives`                       | Optional `apiary_id` filter, status flags, and active diseases — also called directly by VC-08-BE for hive-name resolution (§3.3)                  |
| AST-03-BE | `BE`  | `[x]`  | Tools: `list_inspections`/`list_treatments`/`list_harvests`/`list_feedings` + `get_hive_summary` | Deviates from the original single-tool sketch: `Treatment` has no active/inactive concept in the schema, so each record type got its own `(hive_id, days?)` tool instead of a fixed "latest"/"active" shape, and `get_hive_summary` aggregates all four plus status flags/diseases over the same optional day window |
| AST-04-BE | `BE`  | `[x]`  | Tools: `list_untreated_hives`/`list_uninspected_hives`/`list_unfed_hives` | Widened from a single tool to one per record type, mirroring AST-03-BE's `(apiary_id?, days?)` shape instead of an explicit `since` date — a hive with no record in the last N days, or never had one at all if `days` is omitted (§3.3) |
| AST-05-BE | `BE`  | `[ ]`  | Tool: `list_hives_needing_food`          | Trusts `needs_food` flag initially (§3.3, §5 open question)                                                     |
| AST-06-BE | `BE`  | `[ ]`  | Tool: `compare_hives`                    | Side-by-side metrics for a set of hive IDs (§3.3)                                                                |
