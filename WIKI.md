...
| `app/lib/features/treatment/view/treatment_summary.dart` | Shared `TreatmentSummary` widget — date, treated-by (if not the current user), medicine + dose, notes; extracted out of `HiveDetailScreen` so it can also be reused on the apiary grid screen |
| `app/lib/features/harvest/view/harvest_summary.dart` | Shared `HarvestSummary` widget, same extraction as `TreatmentSummary` — used by both `HiveDetailScreen` and the apiary grid screen |
| `app/lib/features/feeding/view/feeding_summary.dart` | Shared `FeedingSummary` widget, same extraction as `TreatmentSummary` — used by both `HiveDetailScreen` and the apiary grid screen |
...
| POST | `/api/v1/assistant/messages` | AI apiary assistant chat turn — streams an OpenRouter agent-loop response back as Server-Sent Events; only registered if `OPENROUTER_API_KEY` is configured; a conversation is capped at 10 logged messages, after which further posts are rejected |
...
Both the error and no-action rows get a dismiss (×) button that calls the reject endpoint (`POST /voice-recordings/{id}/reject`) and removes the row from the list on success. Tapping a normal proposal row opens a detail dialog showing the full transcript and each proposed action with a friendly tool-name label; each action's arguments render using the same summary widgets as the hive detail screen (`InspectionSummary`/`TreatmentSummary`/`HarvestSummary`/`FeedingSummary`) for the four recognized tools, with a plain field-by-field fallback for anything else (e.g. a hive status update) — plus working Reject (always shown) and Accept (shown only if there's a proposed action to accept) buttons — on success the recording disappears from the "Ready for review" list and the mic icon's badge count updates immediately, not just when the whole recording dialog is closed. Rejecting a recording that actually resolved a hive (a normal proposal, as opposed to an error/no-action row) requires solving the same math-puzzle confirmation used elsewhere for consequential deletes (below) before the reject goes through. Once a recording has been accepted, each successfully-applied action in that detail dialog is tappable and opens the resulting inspection/treatment/harvest/feeding (or the hive edit screen, for a status update) in its normal edit form. Before acceptance, all five proposal types are tappable — `create_inspection`/`create_treatment`/`create_harvest`/`create_feeding` open the matching edit form pre-filled from the proposed arguments, and `update_hive_status` opens a dedicated form pre-filled from the proposal (falling back to the hive's current values for anything the proposal doesn't mention) letting the user edit the four status flags and active diseases; saving any of these updates the proposal itself (rather than creating a record) and the detail dialog reflects the edit immediately, still open underneath.
```
