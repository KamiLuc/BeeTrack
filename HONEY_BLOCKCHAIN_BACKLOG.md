...
| HC-BE-05  | `BE`  | `[x]`  | Blockchain reader                    | `VerifyBatch(...)` + `GetTransactionStatus(txHash)` (confirmed/reverted/blockNumber/gasUsed) — used by the confirmation-polling loop (HC-BE-15c). `GetCertification(batchID)` also consumed directly by the public verification page's live hash check (HC-FE-04), via a small `ChainCertReader` interface so the handler is testable without a real RPC connection.  |
...
| HC-BE-19  | `BE`  | `[x]`  | Handler: GET /api/v1/verify/{token}                      | Renamed from `/honey-batches/{id}/verify`. Public, token-scoped. Returns full lifecycle status, not a boolean. Response shape split into a shared `publicHoneyBatchJSON` (this endpoint: no numeric `id`, no `certification_request`, adds `metadata_hash`/`verification_url`) vs `honeyBatchJSON` (owner endpoints: adds `id` + `certification_request`). `certificationJSON` gained `chain_id`/`contract_address`. See HC-FE-04 for the plain-HTML public page this JSON endpoint sits alongside. |
...
| HC-FE-04 | `FE`  | `[x]`  | Honey batch verification screen           | **Architecture change:** not a Flutter screen. Deliberately moved server-side — the backend now serves the public verification page itself as self-contained HTML (`GET /verify/{token}` on the Go backend, `honey_batch_verify_page.go`, `html/template`, no JS/SPA), so the URL encoded in the QR code opens directly in any browser, including one launched by a phone's stock camera app scanning the QR, without depending on a deployed Flutter web frontend. `HoneyBatchService.VerificationURL(token)` builds this URL from `cfg.APIURL` (the backend's own URL) rather than `cfg.AppURL`. Bilingual via `?lang=pl`/`en` or `Accept-Language` sniffing, defaulting to English. Shows honey type, processing method, gathering date, amount, batch ID (token), certification status, PDF/metadata hashes, and once confirmed, contract address, block number, transaction hash, and a Polygon Amoy explorer link. Once confirmed, the page also does a live on-chain read (`blockchain.HoneyCertReader.GetCertification`, 5s timeout) comparing the stored PDF/metadata hashes against the contract's record, showing a match/mismatch/unavailable badge under each hash; degrades to "unavailable" if blockchain isn't configured or the RPC call fails, never hard-failing the page. See `docs/api.md`. |
...
| HC-FE-06 | `FE`  | `Dropped` | QR code scanner screen                | **Dropped:** no longer planned. Since the QR code simply encodes a universally-openable link to the server-rendered public verification page (see HC-FE-04), any phone's stock camera app can open it directly — an in-app scanner adds no value. |
...
| HC-FE-12 | `FE`  | `Dropped` | QR code scanner util                     | **Dropped:** no longer planned, alongside HC-FE-06 — no in-app scanner means no token-extraction util is needed.          |
